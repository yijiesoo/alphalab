...
def run_single_ticker_backtest(
    ...
    """
    Run the factor backtest for a focused universe and stream progress via *log_fn*.

    Parameters
    ----------
    ticker   : equity symbol the user is focused on, e.g. ``"NVDA"``
    log_fn   : callable that receives one progress string per step
    ...
    cost_bps : one-way transaction cost in basis points (default 10)
    tickers  : universe to run the backtest on; defaults to DEFAULT_TICKERS.
               Pass a smaller list to speed up the stream endpoint.
    ...
    log_fn(f"[stream] Starting backtest for {ticker} ...")

    universe = tickers if tickers is not None else None  # None → download_prices uses DEFAULT_TICKERS
    log_fn(f"[stream] Downloading prices ({start} → {end or 'today'}) ...")
    try:
        prices = download_prices(tickers=universe, start=start, end=end) if universe else download_prices(start=start, end=end)
    except Exception as e:
        log_fn(f"[stream] ERROR downloading prices: {e}")
        raise

    if ticker not in prices.columns:
        log_fn(f"[stream] NOTE: {ticker} not in universe; running on full universe.")

    log_fn(f"[stream] Universe: {prices.shape[1]} stocks, {prices.shape[0]} trading days.")
    ...
    try:
        ...
        log_fn(f"[stream] ERROR running backtest: {e}")
        raise
