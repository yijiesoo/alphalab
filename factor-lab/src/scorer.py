...
def _calculate_entry_exit_levels(prices_series: pd.Series, current_price: float) -> dict:
    ...
    try:
        ...
        return {
            ...
            "summary": "Could not calculate entry/exit zones.",
            "disclaimer": "These are technical watch zones for context, not buy/sell recommendations.",
            "error": str(e),
        }
...
def _calculate_stock_correlation(ticker: str, compare_to: list = None) -> dict:
    ...
    try:
        ...
        return {
            ...
            "overall_interpretation": "Could not calculate correlations.",
            "error": str(e),
        }


def analyze_ticker(ticker: str, sentiment: dict = None) -> dict:
    """
    ...
    sentiment : dict, optional
        Pre-fetched sentiment dict (from cache). If None, fetches fresh from NewsAPI.
    ...
    # 5. Get sentiment — use provided cache entry if available, otherwise fetch fresh
    ...
    data_warnings = []
    if factor.get("error"):
        data_warnings.append(f"Factor score calculation failed: {factor['error']}")
