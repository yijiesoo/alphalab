...
# Demo whitelist — limits yfinance calls on free hosting
DEMO_TICKERS = {
    "AAPL", "NVDA", "TSLA", "MSFT", "GOOGL",
    "META", "AMZN", "JPM", "NFLX", "AMD"
}
...
def api_analyze():
    ...
    try:
        ...
        if ticker in _sentiment_cache:
            sent_entry = _sentiment_cache[ticker]
            if sent_entry["expires_at"] > now:
                cached_sentiment = sent_entry["data"]
                app.logger.info(f"Sentiment cache hit for {ticker}")

        app.logger.info(f"Starting analysis for {ticker}")
        data = analyze_ticker(ticker, sentiment=cached_sentiment)
        data = add_factor_delay_context(data)
        app.logger.info(f"Analysis complete for {ticker}")

        # Store fresh sentiment in cache for future calls
        if data.get("sentiment"):
            _sentiment_cache[ticker] = {
                "data": data["sentiment"],
                "expires_at": now + _SENTIMENT_CACHE_TTL_SECONDS,
            }
...
def api_backtest_stream():
    ...
    if ticker not in DEMO_TICKERS:
        return jsonify({
            "error": f"{ticker} is not available in the demo. Try: AAPL, NVDA, TSLA, MSFT, GOOGL, META, AMZN, JPM, NFLX, or AMD."
        }), 400
    ...
    def generate():
        ...
        def _run() -> None:
            try:
                ...
                results = run_single_ticker_backtest(ticker, log_fn=_log_fn)
                # Compose summary: $1,000 → $X over N months, vs S&P 500
                import numpy as np
                import pandas as pd
                returns = results.get("returns")
                if returns is not None and isinstance(returns, (pd.Series, np.ndarray)):
                    n_months = max(1, int(len(returns) / 21))
                    final_value = (1 + returns).cumprod().iloc[-1]
                    pct = (final_value - 1) * 100
                    # Try to get S&P 500 returns for same period
                    spy = results.get("prices", pd.DataFrame()).get("SPY")
                    if spy is not None:
                        spy_ret = spy.pct_change().dropna()
                        spy_final = (1 + spy_ret).cumprod().iloc[-1]
                        spy_pct = (spy_final - 1) * 100
                        beat = pct - spy_pct
                        summary = f"SUMMARY: This strategy turned $1,000 into ${final_value*1000:.0f} over {n_months} months, beating the S&P 500 by {beat:.1f}%"
                    else:
                        summary = f"SUMMARY: This strategy turned $1,000 into ${final_value*1000:.0f} over {n_months} months."
                    _log_fn(summary)
            except Exception as exc:
                _log_fn(f"ERROR: {exc}")
            finally:
